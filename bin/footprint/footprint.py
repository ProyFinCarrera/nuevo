...
class Footprint:
  ...
  pyfingerprint class.
  Attributes:
    fingerprint (:obj:PyFingerprint()): Obj that
    ...
    try:
      ...
      if aux:
        ...
        if self.is_footprint_equal():
          ...
          if check:
            vect = self.id_footprint(
                pos, buffer=0x01).decode("ASCII")
            vect_aux = codify.des_aes(vect).decode("ASCII")
            # print(vect_aux)
            # print()
            for aux_v in json_v_caracteristic:
              val = json_v_caracteristic[aux_v]
              vect_aux2 = codify.des_aes(val)
              vect_aux2 = vect_aux2.decode("ASCII")
              # print(vect_aux2)
